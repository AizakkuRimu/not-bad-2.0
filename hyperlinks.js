...
(function() {
  ...
  */
  ...
  /* Open Frieren panel instead of the raw overlay for Insert and Auto links.
     These listeners are registered inside hyperlinks.js DOMContentLoaded so they
     run BEFORE uiwiring.js's DOMContentLoaded (same event, earlier script).
     stopImmediatePropagation prevents the raw overlay from opening. */

  function openFrierenFromHyperlinks(mode) {
    var panel = document.getElementById('frierenPanel');
    if (!panel) return;
    /* Close other bot panels */
    ['maomaoPanel','tetiaPanel','cocoPanel'].forEach(function(id) {
    ...
    if (!wasOpen) {
      ...
      /* showFrierenMode is defined in uiwiring.js; call via exposed global */
      if (window._NB_showFrierenMode) window._NB_showFrierenMode(mode);
    } else {
  ...
  _hlInsertBtn.addEventListener('click', function(e) {
    ...
    openFrierenFromHyperlinks('insert');
  });
  _hlAutoBtn.addEventListener('click', function(e) {
    e.stopImmediatePropagation();
    openFrierenFromHyperlinks('auto');
  });
