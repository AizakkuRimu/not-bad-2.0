...
(function () {
  ...
  function shouldIgnore(s) {
    ...
    /* Only apply boilerplate patterns to short sentences (≤ 10 words).
       Longer sentences starting with these phrases can still be meaningful
       and should appear in the review queue. */
    if (words.length <= 10) {
  ...
  /* Cache flattened/marker-ised source text per unique source string, since
     sbGetFixedSentence may be called once per sentence but many sentences
     share the same source letter — flattening is O(length) so without this
     cache it becomes O(sentences × length) and can hang the viewer. */
  var _sbFlatCache = (typeof Map !== 'undefined') ? new Map() : null;
  function _sbGetFlat(sourceText) {
    if (_sbFlatCache && _sbFlatCache.has(sourceText)) return _sbFlatCache.get(sourceText);
    var flat = sourceText
      .replace(/\n[ \t]*\n+/g, '\u0001')
      .replace(/\s+/g, ' ')
      .trim();
    if (_sbFlatCache) _sbFlatCache.set(sourceText, flat);
    return flat;
  }
  ...
  function sbGetFixedSentence(storedText, sourceText) {
    ...
    var flat = _sbGetFlat(sourceText);
  ...
  window.sbOnTabOpen = function () {
    renderTaxonomy();
    rebuildQueue();          /* must rebuild before checking queue length */
    updateQueueBadge();
    renderViewer();
    updateViewerSchemeFilter();
    if (!SB_currentKey && SB_queue.length > 0) {
      /* Pending sentences exist but none is displayed yet — show one */
      showNextSentence();
    } else if (!SB_currentKey && SB_queue.length === 0) {
      elReviewEmpty.style.display = '';
      elReviewCard.style.display = 'none';
    }
  };
