...
(function () {
  ...
  var _subEnqValue   = '';     /* currently selected value ('' = none) */
  var _subEnqOptions = [];     /* string[] of available sub-enquiries */
  ...
  function sbSubEnqSetValue(val) {
    _subEnqValue = val || '';
    if (elSubEnqDispTxt) {
      elSubEnqDispTxt.textContent = _subEnqValue || '— None —';
    }
    if (_subEnqOpen) sbSubEnqClose();
  }

  /* Public getter used by Categorise button */
  ...
  /* sbSaveSubEnquiry — save new sub-enquiry and re-populate */
    var scheme   = elSchemeSelect.value;
    var category = elCategorySelect.value;
    var enquiry  = elEnquirySelect.value;
