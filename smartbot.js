    ...
    if (rec.scheme) {
      ...
      if (rec.category) {
        ...
        if (rec.enquiry) {
          ...
          onEnquiryChange();
          if (rec.subEnquiry) {
            /* Restore the searchable combo selection */
            elSubEnqHidden.value = rec.subEnquiry;
            elSubEnqSearch.value = rec.subEnquiry;
          }
        }
      }
    }
      ...
      Object.keys(payload.db).forEach(function (k) {
        ...
        if (SB_db[k]) {
          ...
          if (r.status === 'categorised' && SB_db[k].status !== 'categorised') {
            ...
            SB_db[k].scheme     = r.scheme     || '';
            SB_db[k].category   = r.category   || '';
            SB_db[k].enquiry    = r.enquiry    || '';
            SB_db[k].subEnquiry = r.subEnquiry || '';
      ...
      (function (subKey, subVal) {
        editBtn.addEventListener('click', function () {
          ...
          Object.keys(SB_db).forEach(function (k) {
            if (SB_db[k].subEnquiry === subVal) {
              var p = subKey.split('||');
              if (SB_db[k].scheme === p[0] && SB_db[k].category === p[1] && SB_db[k].enquiry === p[2]) {
