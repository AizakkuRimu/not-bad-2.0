...
(function () {
  ...
  function renderBatchButtons() {
    ...
    /* Issue 3: export only done (categorised + ignored) sentences */
    var dbKeys = Object.keys(SB_db).filter(function (k) {
      return SB_db[k].status === 'categorised' || SB_db[k].status === 'ignored';
    });
    var total  = dbKeys.length;
    var pendingCount = Object.keys(SB_db).length - total;
    if (totalEl) totalEl.textContent = total + ' done sentence' + (total !== 1 ? 's' : '') +
