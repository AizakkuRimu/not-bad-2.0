...
(function () {
  ...
  function sbGetFixedSentence(storedText, sourceText) {
    ...
    /* Flatten the whole source (line breaks become spaces too) so a sentence
       that wraps across lines, or is interrupted by a line break, can still
       be matched and reconstructed. */
    var flat = sourceText.replace(/\s+/g, ' ').trim();
    ...
    var fixed = norm(flat.slice(start, end));
