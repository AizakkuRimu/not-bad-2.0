...
(function () {
  ...
  function sbGetFixedSentence(storedText, sourceText) {
    ...
    /* Quick check: if it ends with sentence-terminal punctuation it's fine */
    var trimmed = storedText.replace(/\s+$/, '');
    if (/[.!?]$/.test(trimmed)) return null;

    /* Find the stored text inside the source (case-insensitive, normalised ws) */
    var normStored = storedText.replace(/\s+/g, ' ').trim();
    var normSource = sourceText.replace(/\r\n/g, '\n').replace(/\r/g, '\n');

    /* Locate position in source — search normalised */
    var normSourceFlat = normSource.replace(/\s+/g, ' ');
    var pos = normSourceFlat.indexOf(normStored);
    if (pos === -1) {
      /* Try case-insensitive */
      pos = normSourceFlat.toLowerCase().indexOf(normStored.toLowerCase());
    }
    if (pos === -1) return null; /* can't find it, leave alone */

    /* Extract from pos to the next hard line break or sentence-ending punctuation
       followed by a space / end — whichever comes first */
    var after = normSourceFlat.slice(pos);

    /* Find the end of this "sentence" in the source:
       a) end of string
       b) newline (original line break — these are now spaces in normSourceFlat,
          but we can check the original source for the line break)
       c) [.!?] followed by space or end, preceded by a letter */
    /* Re-locate in original (preserve original spacing for the result) */
    var origPos = -1;
    /* Try to find the stored text in the original source preserving case */
    var origSearch = normSource.replace(/[ \t]+/g, ' ');
    origPos = origSearch.indexOf(normStored);
    if (origPos === -1) origPos = origSearch.toLowerCase().indexOf(normStored.toLowerCase());
    if (origPos === -1) return null;

    /* From origPos, find the line this sentence is on */
    var lineStart = origSearch.lastIndexOf('\n', origPos);
    lineStart = (lineStart === -1) ? 0 : lineStart + 1;
    var lineEnd   = origSearch.indexOf('\n', origPos);
    if (lineEnd === -1) lineEnd = origSearch.length;
    var fullLine = origSearch.slice(lineStart, lineEnd).trim();

    /* fullLine is the entire source line containing the sentence.
       Check if the sentence is genuinely a prefix of that line */
    var normLine = fullLine.replace(/\s+/g, ' ').trim();
    var normSt   = normStored.replace(/\s+/g, ' ').trim();

    /* If the full line IS the sentence (or shorter), nothing to fix */
    if (normLine.length <= normSt.length + 2) return null;
    if (normLine.toLowerCase().indexOf(normSt.toLowerCase()) !== 0) return null;

    /* There's more on this line — return the full line as the fixed sentence */
    var fixed = norm(fullLine);
    if (fixed === normStored) return null; /* no change */
    return fixed;
  }
  ...
  function sbGetSourceLetterText(rec) {
    if (!rec.sourceIds || !rec.sourceIds.length) return null;
