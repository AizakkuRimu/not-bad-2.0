...
(function () {
  ...
  function showSentenceDetail(r) {
    ...
    if (r.status === 'categorised') {
      /* Multi-tag: build activeTags from tags array or legacy flat fields */
      var activeTags = (r.tags && r.tags.length) ? r.tags : (r.scheme ? [{ scheme: r.scheme, category: r.category, enquiry: r.enquiry, subEnquiry: r.subEnquiry || '' }] : []);

      if (activeTags.length === 1) {
        html += '<table class="sb-detail-table">';
        if (activeTags[0].scheme)     html += '<tr><td>Scheme</td><td><span class="sb-viewer-tag sb-viewer-tag-scheme">' + escH(activeTags[0].scheme) + '</span></td></tr>';
        if (activeTags[0].category)   html += '<tr><td>Category</td><td><span class="sb-viewer-tag sb-viewer-tag-cat">' + escH(activeTags[0].category) + '</span></td></tr>';
        if (activeTags[0].enquiry)    html += '<tr><td>Enquiry</td><td><span class="sb-viewer-tag sb-viewer-tag-enq">' + escH(activeTags[0].enquiry) + '</span></td></tr>';
        if (activeTags[0].subEnquiry) html += '<tr><td>Sub-Enquiry</td><td><span class="sb-viewer-tag sb-viewer-tag-sub">' + escH(activeTags[0].subEnquiry) + '</span></td></tr>';
        html += '</table>';
      } else if (activeTags.length > 1) {
        html += '<div class="sb-detail-section-label">🏷️ Tags (' + activeTags.length + ')</div>';
        activeTags.forEach(function(tag, ti) {
          html += '<div style="display:flex;align-items:center;flex-wrap:wrap;gap:4px;background:#f8f9ff;border:1px solid #c5d6f8;border-radius:6px;padding:6px 10px;margin-bottom:6px;">';
          html += '<span style="font-size:10px;font-weight:700;color:var(--sf-gray-5);min-width:16px;">#' + (ti+1) + '</span>';
          if (tag.scheme)     html += '<span class="sb-viewer-tag sb-viewer-tag-scheme">' + escH(tag.scheme) + '</span>';
          if (tag.category)   html += '<span class="sb-viewer-tag sb-viewer-tag-cat">' + escH(tag.category) + '</span>';
          if (tag.enquiry)    html += '<span class="sb-viewer-tag sb-viewer-tag-enq">' + escH(tag.enquiry) + '</span>';
          if (tag.subEnquiry) html += '<span class="sb-viewer-tag sb-viewer-tag-sub">' + escH(tag.subEnquiry) + '</span>';
          html += '</div>';
        });
      }
