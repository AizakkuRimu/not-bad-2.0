...
(function () {
  ...
  function sbGetFixedSentence(storedText, sourceText) {
    ...
    var variants = [normStored];
    var stripped = normStored.replace(/\s+\d+\.?\s*$/, '');
    if (stripped && stripped !== normStored) variants.push(stripped);

    for (var v = 0; v < variants.length; v++) {
      var needle = variants[v];

      var origPos = origSearch.indexOf(needle);
      if (origPos === -1) origPos = origSearch.toLowerCase().indexOf(needle.toLowerCase());
      ...
      if (normLine.toLowerCase() === normStored.toLowerCase()) continue;
      if (normLine.length <= needle.length) continue;

      var fixed = norm(fullLine);
      if (fixed && fixed.toLowerCase() !== normStored.toLowerCase()) {
        return fixed;
      }
    }
