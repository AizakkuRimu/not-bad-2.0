    ...
    if (raw.charAt(0) === '{') {
      try {
        ...
        if (imp._caseLib && Array.isArray(imp.cases) && imp.cases.length) {
          ...
          var fixedCases = imp.cases.map(function(c) {
            return {
              id: c.id,
              cells: c.cells.map(function(cell, i) {
                if (i !== c.cells.length - 1) return cell;
                /* Some exports store literal "\n" text instead of real
                   newline characters. Unescape them first, then join
                   any soft-wrapped mid-sentence lines. */
                var fixed = cell.replace(/\\n/g, '\n');
                return joinWrappedLines(fixed);
              })
            };
          });
          TL.cases = TL.cases.concat(fixedCases);
          updateBadge();
          applySearch();
  ...
  */
  ...
  function parseCaseBlocks(rawText) {
    ...
    for (var ci = 0; ci < codes.length; ci++) {
      ...
      /* Find which cell contains "Dear" — this is the start of the
         letter body. All cells from that point onwards are joined as
         the letter. This handles Word table pastes where the letter
         body paragraphs each become separate blank-line-separated
         chunks (causing 16 cells instead of 8).                      */
      var LETTER_COL_IDX = -1;
      for (var di = 0; di < cells.length; di++) {
        if (/\bDear\b/i.test(cells[di])) { LETTER_COL_IDX = di; break; }
      }
      /* Fallback: if no Dear found at all, skip this block */
      if (LETTER_COL_IDX === -1) continue;

      var letterChunk = cells.slice(LETTER_COL_IDX).join('\n\n');

      /* Find "Dear" inside the letter chunk (will be at start now) */
      ...
      /* Build display cells for preview — code + all non-letter cols + letter.
         Non-letter cells get soft-wrap joining and stray-fragment filtering:
         cells that are only punctuation/symbols (e.g. a lone "]") are dropped,
         and adjacent cells that look like they belong together (soft-wrapped
         lines from a Word table paste) are merged with a space.              */
      var displayCells = [codes[ci].code];
      var preCells = [];
      for (var ci2 = 0; ci2 < LETTER_COL_IDX; ci2++) {
        var cv = (cells[ci2] || '').trim();
        /* Drop stray cells that are just punctuation / symbols / very short junk */
        if (/^[^a-zA-Z0-9]*$/.test(cv) && cv.length <= 3) continue;
        preCells.push(cv);
      }
      /* Merge consecutive cells that look like soft-wrapped fragments of the
         same column: if a cell doesn't end with sentence punctuation and the
         next cell starts with a lowercase letter, join them with a space.    */
      var mergedCells = [];
      for (var mi = 0; mi < preCells.length; mi++) {
        var cur = preCells[mi];
        if (mergedCells.length > 0) {
          var prev = mergedCells[mergedCells.length - 1];
          var prevEndsClean = /[.?!:]\s*$/.test(prev);
          var curStartsLower = /^[a-z]/.test(cur);
          if (!prevEndsClean && curStartsLower) {
            mergedCells[mergedCells.length - 1] = prev + ' ' + cur;
            continue;
          }
        }
        mergedCells.push(cur);
      }
      mergedCells.forEach(function(mc) { displayCells.push(mc); });
