...
(function () {
  ...
  window.tlSwitchTab = function (tab) {
    ...
    console.log('[tlSwitchTab] tab=', tab, 'editorEl=', editorEl, 'drawerEl=', drawerEl, 'libEl=', libEl);
    ...
    console.log('[tlSwitchTab] done. viewTemplates classes:', libEl.className);
  };
  ...
  */
  ...
  simClear.addEventListener('click', function () { simInput.value = ''; simResults.innerHTML = ''; simStatus.textContent = ''; simInput.focus(); });
  ...
  function findSimilarCases(queryText) {
    ...
    var scored = TL.cases.map(function (c) {
      /* Only middle columns (not first, not last) for matching — plus last col too
         per spec: "any text in the column that isn't the first or last column"
         but also the last col (the letter body) is very informative. We include
         columns 2 … N (0-indexed: 1 … last). */
      var firstIdx = 0;
  ...
  function runSimSearch() {
    ...
    if (!q) { simStatus.textContent = '⚠️ Please paste some enquiry text first.'; return; }
    if (!TL.cases.length) { simStatus.textContent = '⚠️ No cases loaded yet.'; return; }

    simStatus.textContent = 'Scanning ' + TL.cases.length + ' cases…';
    simResults.innerHTML = '';

    /* Defer so the status renders */
    setTimeout(function () {
      var results = findSimilarCases(q);

      if (!results.length) {
        simStatus.textContent = '⚠️ No similar cases found. Try adding more detail to your query.';
        simResults.innerHTML = '<div class="tl-sim-empty">No matches found — try rephrasing or adding more keywords.</div>';
        return;
      }

      simStatus.textContent = results.length + ' similar case' + (results.length !== 1 ? 's' : '') + ' found. Click a result to load it into the Editor.';

      var topScore = results[0].score || 1;
      var frag = document.createDocumentFragment();

      results.forEach(function (r, i) {
        var pct = Math.round((r.score / topScore) * 100);
        var card = document.createElement('div');
        card.className = 'tl-sim-card';

        var snippet = r.c.cells.slice(1, -1).join(' · ').slice(0, 180);
        if (!snippet) snippet = r.c.cells[r.c.cells.length - 1].slice(0, 180);

        card.innerHTML =
          '<div class="tl-sim-card-hdr">' +
            '<span class="tl-sim-rank">#' + (i + 1) + '</span>' +
            '<div class="tl-sim-score-bar"><div class="tl-sim-score-fill" style="width:' + pct + '%"></div></div>' +
            '<span class="tl-sim-score-pct">' + pct + '%</span>' +
          '</div>' +
          '<div class="tl-sim-title">' + escH(String(r.c.cells[0]).slice(0, 100)) + '</div>' +
          '<div class="tl-sim-snippet">' + escH(snippet) + (snippet.length >= 180 ? '…' : '') + '</div>' +
          (r.shared.length ? '<div class="tl-sim-matches">' +
            r.shared.map(function (w) { return '<span class="tl-sim-match-word">' + escH(w) + '</span>'; }).join('') +
          '</div>' : '');
