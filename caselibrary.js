...
(function () {
  ...
  /* ══════════════════════════════════════════════════════════════
     ...
     6.  The letter runs from "Dear" to the end of the group. Since the
         group is already bounded to just before the next case ID (or
         end of text), no closing-phrase trimming is needed.
  ...
  function parseCaseBlocks(rawText) {
    ...
    for (var ci = 0; ci < codes.length; ci++) {
      ...
      /* Step 4 — split on blank lines (handles lines with only spaces/tabs too) */
      ...
      /* Join all segments from dearIdx onward as the letter.
         The group is already sliced to just before the next case ID,
         so no closing-phrase trimming is needed. */
  ...
  */
  ...
  /* Find the last occurrence of "Dear" in text and return everything from
     that point onward. If "Dear" is not found, returns null. */
  function extractTemplateChunk(text) {
    var dearRe = /\bDear\b/gi;
    var lastIdx = -1;
    var m;
  ...
  /* Normalise whitespace for fuzzy membership check */
  function normWs(s) { return s.replace(/\s+/g, ' ').trim().toLowerCase(); }

  /* Check whether needle appears (approximately) inside haystack.
     We strip whitespace and do a substring search on normalised text. */
  function sentenceInTemplate(sentence, templateChunk) {
    if (!sentence || !templateChunk) return false;
    var needleNorm = normWs(sentence);
    var haystackNorm = normWs(templateChunk);
    if (!needleNorm || needleNorm.length < 5) return false;
    return haystackNorm.indexOf(needleNorm) !== -1;
  }

  /* Highlight all occurrences of sentence inside HTML-escaped text,
     wrapping each match in a <mark> span. Works on plain text before
     escaping so we can do a reliable normalised comparison. */
  function buildHighlightedHtml(cellText, highlightSentence, q) {
    if (!highlightSentence || !cellText) {
      return q ? hlText(cellText, q).replace(/\n/g, '<br>')
               : escH(cellText).replace(/\n/g, '<br>');
    }

    var needleNorm = normWs(highlightSentence);
    var haystackNorm = normWs(cellText);
    if (!needleNorm || haystackNorm.indexOf(needleNorm) === -1) {
      /* Sentence not found — still render normally */
      return q ? hlText(cellText, q).replace(/\n/g, '<br>')
               : escH(cellText).replace(/\n/g, '<br>');
    }

    /* Walk character by character through cellText, matching the
       normalised needle. We need to map normalised positions back to
       original positions (whitespace-collapsed needle vs real text). */
    /* Strategy: find the needle in the normalised haystack, then map
       that normalised range back to original text offsets. */
    var normStart = haystackNorm.indexOf(needleNorm);
    var normEnd   = normStart + needleNorm.length;

    /* Map normalised offset → original offset via a position map */
    function buildNormMap(original) {
      var map = []; /* map[normPos] = origPos */
      var normPos = 0;
      var inWs = false;
      for (var i = 0; i < original.length; i++) {
        var ch = original[i];
        if (/\s/.test(ch)) {
          if (!inWs) { map.push(i); normPos++; inWs = true; }
        } else {
          map.push(i);
          normPos++;
          inWs = false;
    ...
    var posMap = buildNormMap(cellText.toLowerCase());
    var origStart = (normStart < posMap.length) ? posMap[normStart] : -1;
    var origEnd   = (normEnd - 1 < posMap.length) ? posMap[normEnd - 1] + 1 : -1;
    ...
    var before  = cellText.slice(0, origStart);
    var matched = cellText.slice(origStart, origEnd);
    var after   = cellText.slice(origEnd);

    var htmlBefore  = q ? hlText(before, q)  : escH(before);
    var htmlMatched = '<mark class="tl-sentence-hl">' + escH(matched) + '</mark>';
    var htmlAfter   = q ? hlText(after, q)   : escH(after);

    return (htmlBefore + htmlMatched + htmlAfter).replace(/\n/g, '<br>');
  }

  /* ── PREVIEW ────────────────────────────────────────────────── */
  /* Optional: set this before calling showPreview / tlSelectCaseById
     to highlight a specific sentence and show the template indicator. */
  var _previewHighlightSentence = null;

  window.tlSetPreviewSentence = function (sentence) {
    _previewHighlightSentence = sentence || null;
  };
  ...
  function showPreview(c, q) {
    ...
    /* ── Template chunk detection ─────────────────────────────── */
    var lastCell = c.cells[lastIdx] || '';
    var templateChunk = extractTemplateChunk(lastCell);
    var highlightSentence = _previewHighlightSentence;
    ...
    c.cells.forEach(function (cell, ci) {
      ...
      /* For the last column (letter body), add the template indicator */
      if (ci === lastIdx && highlightSentence) {
        var inTemplate = templateChunk && sentenceInTemplate(highlightSentence, templateChunk);
        var badge = document.createElement('span');
        if (inTemplate) {
          badge.className = 'tl-template-badge tl-template-badge-in';
          badge.textContent = '✓ Found in Template';
        } else {
          badge.className = 'tl-template-badge tl-template-badge-out';
          badge.textContent = '✕ Not in Template — can ignore';
        }
        lbl.textContent = '★ Column ' + (ci + 1) + '  (pushed to Editor)  ';
      ...
      if (ci === lastIdx && highlightSentence) {
        /* Use highlight renderer for the letter body */
        val.innerHTML = buildHighlightedHtml(cell, highlightSentence, q);
        /* Scroll the highlighted sentence into view after render */
        setTimeout(function () {
          var mark = val.querySelector('.tl-sentence-hl');
  ...
  /* ── PUBLIC: Expose full case objects (with id) to SmartBot ── */
  window.tlGetAllCasesWithId = function () {
    return TL.cases.map(function (c) {
      return {
        id: c.id,
        cells: c.cells,
        tag: c.tag,
        letter: Array.isArray(c.cells) ? c.cells[c.cells.length - 1] || '' : ''
      };
    });
  };

  /* ── PUBLIC: Select and highlight a case by ID in the list ── */
  window.tlSelectCaseById = function (caseId, highlightSentence) {
    var c = TL.cases.find(function (x) { return x.id === caseId; });
    if (!c) return false;

    /* Store highlight sentence for showPreview to use */
    _previewHighlightSentence = highlightSentence || null;

    /* Switch to Case Library tab */
    tlSwitchTab('templates');

    /* Clear search so the case is visible */
    searchInput.value = '';
    searchInput.disabled = false;
    applySearch();

    /* Wait for render, then find and click the card */
    setTimeout(function () {
      var card = resultsList.querySelector('[data-id="' + caseId + '"]');
      if (card) {
        var prev = resultsList.querySelector('.tl-selected');
        if (prev) prev.classList.remove('tl-selected');
        card.classList.add('tl-selected');
        TL.selectedId = caseId;
