...
(function () {
  ...
  /* ── FILE UPLOAD ────────────────────────────────────────────── */
  var fileInput    = document.getElementById('tlFileInput');
  var openFileBtn  = document.getElementById('tlOpenFile');
  var fileProgress = document.getElementById('tlFileProgress');
  var fileProgBar  = document.getElementById('tlFileProgressBar');
  var fileProgLbl  = document.getElementById('tlFileProgressLabel');
  ...
  fileInput.addEventListener('change', function () {
    ...
    fileProgLbl.textContent = 'Reading file…';

    var reader = new FileReader();

    /* Progress events — FileReader fires these for large files */
    reader.onprogress = function (e) {
      if (e.lengthComputable) {
        var pct = Math.round((e.loaded / e.total) * 80); // reserve last 20% for parse
        fileProgBar.style.width = pct + '%';
      }
    };

    reader.onload = function (e) {
      fileProgBar.style.width = '85%';
      fileProgLbl.textContent = 'Parsing cases…';

      /* Slight defer so the browser can repaint the progress bar */
      setTimeout(function () {
        var text = e.target.result;

        /* Detect format and count cases before committing */
        var isColumnFormat = /^Column\s*\(1\)\s*:/m.test(text);
        var previewCases   = isColumnFormat
          ? parseColumnFormat(text)
          : parseCaseBlocks(text);

        fileProgBar.style.width = '100%';

        if (!previewCases.length) {
          fileProgLbl.textContent = '⚠️ No cases found in file.';
          setTimeout(function () { fileProgress.style.display = 'none'; }, 3000);
          return;
        }

        /* Show count and confirm */
        fileProgLbl.textContent = '✅ ' + previewCases.length + ' case(s) found';

        /* Load them in */
        var startId = TL.cases.length;
        previewCases.forEach(function (cells, i) {
        ...
        showToast('📂 ' + previewCases.length + ' case(s) loaded from file — ' + TL.cases.length + ' total', 'success');

        setTimeout(function () { fileProgress.style.display = 'none'; }, 2500);
      }, 60);
    };

    reader.onerror = function () {
      fileProgLbl.textContent = '❌ Failed to read file.';
      fileProgBar.style.background = '#e74c3c';
      setTimeout(function () { fileProgress.style.display = 'none'; }, 3000);
    };
  ...
  function resetPasteBoxes() {
    ...
    var boxes = container.querySelectorAll('.tl-paste-box');
  ...
  function renumberBoxLabels() {
    var boxes = document.getElementById('tlPasteBoxes').querySelectorAll('.tl-paste-box');
    boxes.forEach(function (box, i) {
