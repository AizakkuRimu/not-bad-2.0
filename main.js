...
function copyOutputClean() {
  ...
  // Salesforce needs <p> tags with inline margin for spacing — <br> does NOT create
  // visual paragraph spacing in Salesforce's email composer. We convert the editor's
  // div-per-line structure into proper <p style="margin:0 0 10pt 0"> blocks so that
  // pasted text already has the same spacing Salesforce applies when you
  // bullet-then-unbullet manually.

  const clone = output.cloneNode(true);
  const P_STYLE = 'margin:0 0 10pt 0;';

  // ── Build inline HTML ──────────────────────────────────────
  // Preserves: <a>, <b>/<strong>, <i>/<em>, <u>, <span style="color:...">
  // Strips: highlights/marks, unstyled spans, <br> (handled at block level)
  function inlineHTML(node) {
    ...
    const kids = () => Array.from(node.childNodes).map(inlineHTML).join('');
    if (tag === 'b' || tag === 'strong') return `<b>${kids()}</b>`;
    if (tag === 'i' || tag === 'em')     return `<i>${kids()}</i>`;
    if (tag === 'u')                     return `<u>${kids()}</u>`;
    if (tag === 'br')                    return ''; // handled at block level
    if (tag === 'a') {
      const href = (node.getAttribute('href') || '').replace(/"/g,'&quot;');
      return `<a href="${href}">${kids()}</a>`;
    }
    if (tag === 'span' && node.style.color) {
      return `<span style="color:${node.style.color}">${kids()}</span>`;
    }
    // mark, highlight spans, generic wrappers → just content
    return kids();
  }

  // ── Convert each top-level node into a <p> block or list ──
  function blockHTML(node) {
    if (node.nodeType === Node.TEXT_NODE) {
      const t = node.textContent;
      if (!t) return '';
      return `<p style="${P_STYLE}">${t.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}</p>`;
    }
    if (node.nodeType !== Node.ELEMENT_NODE) return '';
    const tag = node.tagName.toLowerCase();

    // Lists — keep <ul>/<ol> structure with proper spacing
    if (tag === 'ul' || tag === 'ol') {
      const lis = Array.from(node.children)
        .filter(c => c.tagName === 'LI')
        .map(li => `<li>${inlineHTML(li)}</li>`)
        .join('');
      return `<${tag} style="margin:0 0 10pt 0; padding-left:24pt;">${lis}</${tag}>`;
    }

    // Block elements (div, p) → <p> paragraph
    if (tag === 'div' || tag === 'p') {
      const isEmpty = !node.textContent.trim();
      if (isEmpty) return `<p style="${P_STYLE}">&nbsp;</p>`; // blank line = visual spacer
      return `<p style="${P_STYLE}">${inlineHTML(node)}</p>`;
    }

    // Inline elements at root (shouldn't normally appear, but handle gracefully)
    return `<p style="${P_STYLE}">${inlineHTML(node)}</p>`;
  }

  const cleanHTML = Array.from(clone.childNodes).map(blockHTML).filter(Boolean).join('');

  // Plain text fallback (uses browser's own text extraction so line breaks are correct)
...
async function checkGrammar() {
  ...
  try {
    const body = new URLSearchParams({ text, language: 'en-US', disabledCategories: 'CASING,PUNCTUATION,TYPOGRAPHY,STYLE,COLLOCATIONS', ignoredWords: 'YSOA,MediSave,Singpass,SingPass' });
...
(function() {
  ...
  async function getBrowserSpellMisspellings(text) {
    ...
    const IGNORED = new Set(['YSOA','MediSave','Singpass','SingPass','NRICs']);
      ...
      const [ltResp, spellHits] = await Promise.all([
        fetch('https://api.languagetool.org/v2/check', {
          ...
          body: new URLSearchParams({
            ...
            ignoredWords: 'YSOA,MediSave,Singpass,SingPass'
          }).toString()
        }),
