...
function highlightWord(el, word, color) {
  ...
  walkTextNodes(el, node => {
    ...
    span.innerHTML = node.textContent.replace(re, m =>
      `<mark class="word-highlight" style="background:${color};">${escHtml(m)}</mark>`
    );
...
function copyOutputClean() {
  ...
  // Build a clean HTML version: only <a> tags survive, everything else is unwrapped to plain text
  const clone = output.cloneNode(true);

  // Unwrap all marks/highlights/spans — keep their text content
  clone.querySelectorAll('mark, span, b, strong, em, i, u, div, p, br').forEach(el => {
    if (el.tagName === 'BR') {
      el.replaceWith('\n');
    } else {
  ...
  // Now clone only has text nodes and <a> tags
  // Build clean HTML: text escaped, <a> tags kept as-is
