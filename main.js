...
function copyOutputClean() {
  ...
  // Unwrap only marks/highlights/unstyled spans/block elements — keep formatting tags and list structure
  // Process deepest elements first (reverse document order) to avoid re-visiting
  const unwrapTargets = Array.from(clone.querySelectorAll('mark, span, div, p, br'));
  unwrapTargets.reverse().forEach(el => {
    if (el.tagName === 'BR') {
      el.replaceWith('\n');
    } else if (el.tagName === 'SPAN' && el.style.color) {
      // Colour span — convert to a font tag so colour survives into the clipboard HTML
      const font = document.createElement('font');
      font.setAttribute('color', el.style.color);
  ...
  // Now clone only has text nodes, <a> tags, formatting tags, and list tags (ul/ol/li)
  // Build clean HTML: text escaped, <a>, formatting, and list tags kept
  function buildCleanHTML(node) {
    ...
    if (node.nodeType === Node.ELEMENT_NODE) {
      ...
      // Formatting tags — wrap children with the same tag
      if (['b','strong','em','i','u'].includes(tag)) {
        const inner = Array.from(node.childNodes).map(buildCleanHTML).join('');
        return inner ? `<${tag}>${inner}</${tag}>` : '';
      }
      // Colour (converted from span[style=color] above)
      if (tag === 'font' && node.getAttribute('color')) {
        const inner = Array.from(node.childNodes).map(buildCleanHTML).join('');
        return inner ? `<font color="${node.getAttribute('color').replace(/"/g,'&quot;')}">${inner}</font>` : '';
      }
      // Hyperlinks
      if (tag === 'a') {
