...
document.addEventListener('DOMContentLoaded', function() {
  ...
  /* ── Helper: position panel so its bottom-left corner anchors
  ...
  function positionPanelAtButton(panel, btn) {
    ...
    var top = rect.bottom; /* panel will open upward via CSS transform-origin */
    /* Clamp to viewport */
    if (left < 8) left = 8;
    /* We use bottom anchor: set bottom = viewport height - rect.bottom, left = rect.right - panel width */
    var bottom = window.innerHeight - rect.bottom;
    panel.style.bottom = Math.max(bottom, 8) + 'px';
