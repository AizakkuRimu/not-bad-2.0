...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function closeAllBotPanels() {
    ...
    /* Also close standalone Smart Link panel */
    var slP = document.getElementById('slPanel');
    if (slP) slP.classList.remove('open');
    var btnSL = document.getElementById('btnSmartLink');
    if (btnSL) btnSL.classList.remove('sl-btn-active');
  }
  ...
  function positionPanelAtButton(panel, btn) {
    ...
    /* Dynamically cap max-height to available space below final top position */
    var availH = window.innerHeight - 8 - top;
    if (availH < panelH) {
      panel.style.maxHeight = Math.max(180, availH) + 'px';
    } else {
  ...
  /* ── Smart Link in Coco quick-bar → delegates to toolbar button ── */

  document.getElementById('cocoSmartLink').addEventListener('click', function() {
    /* Close Coco panel and open the dedicated Smart Link panel */
    document.getElementById('cocoPanel').classList.remove('open');
    document.getElementById('cocoPanel').classList.remove('expanded');
    var btnSL = document.getElementById('btnSmartLink');
    if (btnSL) btnSL.click();
  });

  /* ── Standalone Smart Link Panel (toolbar button) ─────── */

  var slPanelMode = 'word'; /* 'word' | 'sentence' */
  var slPanel = document.getElementById('slPanel');
  var btnSmartLink = document.getElementById('btnSmartLink');

  function openSlPanel() {
    closeAllBotPanels();
    if (btnSmartLink) positionPanelAtButton(slPanel, btnSmartLink);
    slPanel.classList.add('open');
    if (btnSmartLink) btnSmartLink.classList.add('sl-btn-active');
  }

  function closeSlPanel() {
    slPanel.classList.remove('open');
    slPanel.classList.remove('has-results');
    if (btnSmartLink) btnSmartLink.classList.remove('sl-btn-active');
  }

  if (btnSmartLink) {
    btnSmartLink.addEventListener('click', function() {
      if (slPanel.classList.contains('open')) {
        closeSlPanel();
      } else {
        openSlPanel();
      }
  ...
  document.getElementById('slPanelClose').addEventListener('click', closeSlPanel);

  document.getElementById('slModeWord').addEventListener('click', function() {
    slPanelMode = 'word';
    document.getElementById('slModeWord').classList.add('active');
    document.getElementById('slModeSentence').classList.remove('active');
    document.getElementById('slWordOpt').style.display = '';
  });

  document.getElementById('slModeSentence').addEventListener('click', function() {
    slPanelMode = 'sentence';
    document.getElementById('slModeSentence').classList.add('active');
    document.getElementById('slModeWord').classList.remove('active');
    document.getElementById('slWordOpt').style.display = 'none';
  });

  document.getElementById('slRun').addEventListener('click', function() {
    if (window._NB_rebuildCorpus) window._NB_rebuildCorpus();

    var corpus = window._NB_corpus;
    if (!corpus || !corpus.length) {
      showToast('⚠️ Case Library is empty — paste some cases first!', 'error');
      return;
    }
    ...
    /* Get context using current mode */
    var contextText = getSlPanelContext(editor, slPanelMode);
    if (!contextText) {
      showToast('⚠️ Click inside the editor first, then try again.', 'error');
      return;
    }

    var results = findSmartLinkSuggestions(contextText, corpus);

    var slQuery = document.getElementById('slQuery');
    var slList  = document.getElementById('slList');
    var slFooter = document.getElementById('slFooter');

    slQuery.style.display = '';
    slQuery.innerHTML =
      'Context: <strong>"' + escHtml(contextText.slice(0, 70)) + (contextText.length > 70 ? '…' : '') + '"</strong><br>' +
      '<span style="font-size:10px;color:#7cb87c;">' + results.length + ' suggestion' + (results.length !== 1 ? 's' : '') + ' · ' + corpus.length + ' sentences indexed</span>';

    if (!results.length) {
      slList.innerHTML = '<div style="padding:16px;text-align:center;color:var(--sf-gray-5);font-size:12px;">No matching follow-up sentences found.<br>Try fewer words or switch to SENTENCE mode.</div>';
    ...
    slPanel.classList.add('has-results');

    /* Re-position to account for expanded height */
    if (btnSmartLink) positionPanelAtButton(slPanel, btnSmartLink);
    ...
    slList.querySelectorAll('.coco-sim-use').forEach(function(btn) {
      btn.addEventListener('click', function(e) {
        e.stopPropagation();
        var idx = parseInt(btn.dataset.idx);
        insertSmartLinkSentence(results[idx].text, editor);
        closeSlPanel();
      });
    });
  });

  function getSlPanelContext(editor, mode) {
    var sel = window.getSelection();
    ...
    if (mode === 'sentence') {
      var sentences = textBefore.split(/([.!?])\s+/).reduce(function(acc, part, i) {
        if (i % 2 === 0) { acc.push(part); } else { acc[acc.length - 1] += part; }
        return acc;
      }, []);
      return sentences[sentences.length - 1].trim() || textBefore.trim();
    } else {
      var wordCount = parseInt(document.getElementById('slWordCount').value) || 5;
  ...
  function findSmartLinkSuggestions(contextText, corpus) {
    ...
    */
    ...
    tl.cases.forEach(function(c) {
      ...
      var sents = caseText
        .replace(/\r\n/g, '\n')
        .split(/([.!?])\s+/).reduce(function(acc, part, i) {
          if (i % 2 === 0) { acc.push(part); } else { acc[acc.length - 1] += part; }
          return acc;
        }, [])
  ...
  (function() {
    ...
    var btnBold      = document.getElementById('fmtBold');
    var btnItalic    = document.getElementById('fmtItalic');
    var btnUnderline = document.getElementById('fmtUnderline');
    var btnBullet    = document.getElementById('fmtBullet');
    var btnNumbered  = document.getElementById('fmtNumbered');
    var colorGroup   = document.querySelector('.fmt-color-group');
    var colorPalette = document.getElementById('fmtColorPalette');
    var colorSwatch  = document.getElementById('fmtColorSwatch');
    var activeColor  = '';   // currently selected colour
    ...
    function toggleList(type) {
      editor.focus();
      var cmd = type === 'bullet' ? 'insertUnorderedList' : 'insertOrderedList';
      document.execCommand(cmd, false, null);
      /* Remove white-space:pre-wrap while lists are present so they render properly */
      var hasLists = editor.querySelector('ul, ol');
      editor.style.whiteSpace = hasLists ? 'normal' : '';
      updateActiveStates();
    }
    ...
    function applyColor(color) {
      ...
      if (color) {
        ...
        /* Remove colour — use removeFormat on foreColor or set to inherit */
        document.execCommand('foreColor', false, 'inherit');
        colorSwatch.style.background = 'var(--sf-gray-7)';
      }
      activeColor = color;
    ...
    function updateActiveStates() {
      btnBold.classList.toggle('fmt-active',      document.queryCommandState('bold'));
      btnItalic.classList.toggle('fmt-active',    document.queryCommandState('italic'));
      btnUnderline.classList.toggle('fmt-active', document.queryCommandState('underline'));
      btnBullet.classList.toggle('fmt-active',    document.queryCommandState('insertUnorderedList'));
      btnNumbered.classList.toggle('fmt-active',  document.queryCommandState('insertOrderedList'));
    }

    /* ── Button click handlers ────────────────────────────── */
    btnBold.addEventListener('mousedown',      function(e) { e.preventDefault(); fmt('bold'); });
    btnItalic.addEventListener('mousedown',    function(e) { e.preventDefault(); fmt('italic'); });
    btnUnderline.addEventListener('mousedown', function(e) { e.preventDefault(); fmt('underline'); });
    btnBullet.addEventListener('mousedown',    function(e) { e.preventDefault(); toggleList('bullet'); });
    btnNumbered.addEventListener('mousedown',  function(e) { e.preventDefault(); toggleList('number'); });

    /* ── Colour palette open/close ────────────────────────── */
    colorGroup.addEventListener('mousedown', function(e) {
      e.preventDefault();
    ...
    /* ── Colour chip clicks ───────────────────────────────── */
    colorPalette.querySelectorAll('.fmt-color-chip').forEach(function(chip) {
    ...
    /* ── Keep active states in sync with cursor movement ───── */
    ...
    /* ── Keyboard shortcuts already handled by browser
          but make sure active states update on them ────────── */
