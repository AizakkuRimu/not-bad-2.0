...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var fmtColorDropdown = document.getElementById('fmtColorDropdown');
  var fmtColorA = document.getElementById('fmtColorA');
  var inputEditor = document.getElementById('inputEditor');

  if (editorFormatBar) {
    /* Prevent toolbar clicks from stealing focus away from editor */
    editorFormatBar.addEventListener('mousedown', function(e) {
      if (!e.target.matches('input, textarea')) e.preventDefault();
    });

    /* Bold / Italic / Underline / List buttons */
    editorFormatBar.querySelectorAll('.fmt-btn[data-cmd]').forEach(function(btn) {
      btn.addEventListener('click', function() {
        document.execCommand(btn.dataset.cmd, false, null);
        if (inputEditor) inputEditor.dispatchEvent(new Event('input'));
        updateFmtStates();
      });
    });

    /* Colour trigger button — toggle dropdown */
    document.getElementById('fmtColorBtn').addEventListener('click', function(e) {
      e.stopPropagation();
      fmtColorDropdown.classList.toggle('open');
    });

    /* Colour swatches */
    document.querySelectorAll('.fmt-color-swatch').forEach(function(swatch) {
      /* mousedown so selection is still live when we call execCommand */
      ...
      swatch.addEventListener('click', function(e) {
        ...
        if (color === 'reset') {
          ...
          document.execCommand('foreColor', false, '#3e3e3c');
          fmtColorA.style.borderBottomColor = '#201f1e';
        } else {
          document.execCommand('foreColor', false, color);
          fmtColorA.style.borderBottomColor = color;
        }
    ...
    /* Close colour dropdown on any outside click */
    document.addEventListener('click', function(e) {
      if (!e.target.closest('#fmtColorWrap')) {
        fmtColorDropdown.classList.remove('open');
      }
    });

    /* Update active states when cursor moves / selection changes */
    function updateFmtStates() {
      ['bold', 'italic', 'underline', 'insertUnorderedList', 'insertOrderedList'].forEach(function(cmd) {
        var btn = editorFormatBar.querySelector('.fmt-btn[data-cmd="' + cmd + '"]');
        if (!btn) return;
    ...
    if (inputEditor) {
      inputEditor.addEventListener('keyup', updateFmtStates);
      inputEditor.addEventListener('mouseup', updateFmtStates);
    }
    document.addEventListener('selectionchange', function() {
      if (document.activeElement === inputEditor) updateFmtStates();
    });
  }
